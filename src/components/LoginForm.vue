<template>
    <div class="login">
    <CardUI class="login-card">
        <template #title>
            <div class="login-title">
                <h3 class="login-title__text">Login</h3>
            </div>
        </template>
        <div class="login-content">
            <InputUI class="login-input__text" v-model="user.email" label="Username"></InputUI>
            <InputUI class="login-input__password" v-model="user.password" label="Password"></InputUI>
            <span @click="handleForgotPassword" class="login-text">forgot password?</span>
        </div>
        <template #actions>
            <div class="login-actions">
                <ButtonUI class="action__btn" @click.stop="handleLogin">Login</ButtonUI>
                <span class="action__text">Or sign Up Using</span>
                <div class="action__icons">
                    <Github class="w-6 h-6 text-gray-600" />
                    <span>yandex</span>
                    <span>google</span>
                </div>
            </div>
        </template>
    </CardUI>
    </div>
</template>

<script setup lang="ts">
import ButtonUI from '../ui-kit/src/components/Button.vue';
import InputUI from '../ui-kit/src/components/Input.vue';
import CardUI from '../ui-kit/src/components/Card.vue';
import { reactive } from 'vue';
import { Github, AlignVerticalSpaceAround } from 'lucide-vue-next';

interface User {
    email: string;
    password: string;
}

const user = reactive<User>({
    email: '',
    password: '',
});

const handleLogin = () => {
    
}

const handleForgotPassword = () => {

}
</script>

<style scoped lang="scss">
.login {
    display: flex;
    align-items: center;
    justify-content: center;
    height: 100vh;

    &-card {
        width: 30%;
        height: auto;
    }
}

.login-content {
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: space-between;
    width: 100%;
}

.login-text {
    width: 100%;
    text-align: center;
    text-decoration: none;
    transition: text-decoration 0.2s ease;
    &:hover {
        text-decoration: underline;
        cursor: pointer;
    }
}

.login-input__text {
    width: 100%;
}

.login-input__password {
    width: 100%;
}


.login-title {
    width: 100%;
    text-align: center;
}

.login-actions {
    width: 100%;
    gap: 8px;
    display: flex;
    flex-direction: column;
    align-items: center;
}

.action__btn {
    width: 100%;
}

.action__text {
    font-size: 14px;
}

.action__icons {
    width: 100%;
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 16px;
}

</style>